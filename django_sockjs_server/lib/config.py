...
class SockJSServerSettings(object):
    ...
    def __init__(self, options=None):
        ...
        conf = getattr(settings, 'DJANGO_SOCKJS_SERVER', None)    # dict
        # self.logger.info(conf)
        if not conf:
            raise Exception('django-sockjs-server error! No settings.')

        self.rabbitmq_user = conf.get('rabbitmq_user', 'guest')    # get value via key, if not, return default
        ...
        self.rabbitmq_queue1_name = conf.get('rabbitmq_queue1_name', 'MQ1')    # MQ1
        self.rabbitmq_queue2_name = conf.get('rabbitmq_queue2_name', 'MQ2')    # MQ2
